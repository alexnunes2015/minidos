...
- **Graphics Scratch Buffer (Current)**: The software backbuffer no longer lives in the kernel `.bss`. It uses a dedicated scratch window starting at `0x00400000`, which keeps the early boot stack and GUI frame buffers inside the initial low-memory identity map while the app user window now lives separately at `0x01000000..0x01100000`. The reserved window remains in place, but the fast-present path is currently disabled while the preemptive renderer is hardened.
...
## Current Limitations
- ELF loader is intentionally minimal (ELF32 `ET_EXEC`, fixed 1 MiB user window at `0x01000000`, no relocations/dynamic linking).
...
- The COM loader is still a raw flat-binary path linked into the same fixed user window at `0x01000000`, with no relocations or metadata beyond the file length and the shared runtime block injected by the kernel.
