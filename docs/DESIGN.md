...
- **Userland (Current)**: External ELF32 apps are loaded from FAT16 and executed in protected mode with a growing syscall ABI that now covers console input, graphics primitives, explicit frame presentation, ticks, mouse snapshots/waitable UI events, and app-created child threads. `runbg` launches ELFs as scheduler-managed kernel tasks, child threads inherit the same app image and leader PID, and `kill <pid>` terminates the whole app group.
...
- The scheduler now has real kernel-thread runtime state and guard-page protection for managed stacks, and background ELF apps can run as scheduler-managed tasks with grouped child threads. They still execute as kernel tasks, not isolated ring3 processes with private user memory.
- Foreground ELF/COM execution still reuses the inline shell path, so MiniDOS does not yet have a unified process model where every app always owns a dedicated user/kernel context pair.
...
- Unify foreground and background app launch onto the scheduler runtime so every executable gets the same lifecycle and accounting path.
